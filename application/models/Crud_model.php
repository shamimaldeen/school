...
class Crud_model extends CI_Model {
    ...
    function get_subject_name_by_id($subject_id) {
        $query = $this->db
            ->select('subject.*, teacher.name as teacher_name')
            ->join('teacher','subject.teacher_id = teacher.teacher_id')
            ->get_where('subject', array('subject_id' => $subject_id))->row();
        return ($query) ? $query->name . ' - ('.$query->teacher_name.') - ' : null;
    }
    ...
    function get_exams() {
        ...
        $query = $this->db->get_where('exam' , array('vtype'=>$vtype,
    ...
    function select_study_material_info()
    {
        ...
        return $this->db->get_where('document',['vtype'=>$vtype])->result_array();
    }
    ...
    function update_study_material_info($document_id)
    {
        $data['timestamp']      = strtotime($this->input->post('timestamp'));
        $data['title'] 		= $this->input->post('title');
        $data['title'] 		= $this->session->userdata('vtype');
        $data['description']    = $this->input->post('description');
        $data['class_id'] 	= $this->input->post('class_id');
        $data['subject_id']     = $this->input->post('subject_id');
