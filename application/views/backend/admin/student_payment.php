...
<script type="text/javascript">
    function getTuitionFeeViewData(e) {
        // if (sec_id != '') {
            var selected_sections = $(e.target).val();
            ...
            $.each(selected_sections, function(index, value){
                $.ajax({
                    url: '<?php echo site_url('admin/search_student_payment_status/');?>' + value + '/' + std_id,
                    success: function (response) {
                        jQuery('#monthly-data').append(response);
                    }
                });
            });
        // }
    ...
    //    var std_id = 1;//jQuery.('#student_id').val();
    //    var sec_id = 2;//jQuery.('#section_id').val();
    //    $.ajax({
        ...
        <div id="invoice_print">
            <div class="header_igm col-md-12">
                <table width="100%" border="0" style="text-align: center">
                    <tr>
                        <td>
                            <img style="text-align: center!important; display: inline-block !important;; margin: 0 auto" width="300" src="<?php echo base_url('uploads/large_logo.jpg') ?>" alt="">
                            <br>
                            <br>
                        </td>
                    </tr>
                ...
                <table width="100%"  style="border-collapse:collapse;"class="table_body table table-bordered" border="1">
                    ...
                    foreach ($rows as $row):
                        ...
                        if ($row->payment_type == 2) {
                            $month_tuition = $this->db->select('month, yr')->get_where('tuition_fee_collection' , array(
                            ...
                            $section = $this->db->select('course_type')
                                ...
                                ->where('vtype', $this->session->userdata('vtype'))->get()->row();
                            $courseType = ($section) ? $section->course_type : null;
                        }
...
//                                var_dump($month_transport); exit();
                                        ...
                                        if ($row->payment_type == 2 ){
                                            if ($courseType==2){
                                                ...
                                                if (!empty($month_tuition)) {
                                                    ...
                                                    {
                                                        $monthText .= get_monthNameEdit($m_tuition->month) . ' ' . $m_tuition->yr . ', ';
                                                    }
                                                    echo trim($monthText, ' ,');
                                                }
                                            }
                ...
                <b style="text-transform: capitalize; margin-top: 30px"> Amount in words : <?php echo convertNumber($total_amount.'.00') ?> taka only </b>
            </div>
