...
<hr />
<?php echo form_open(site_url('admin/invoice/do_update') , array('class' => 'form-horizontal form-groups-bordered validate','target'=>'_self'));?>
...
//                        dd($tuition);
                        ...
                        $section = $this->db->select('course_type')
                            ->from('section')
                            ->where('section_id', $root->section_id)
                            ->where('vtype', $vtype)->get()->row();

                        $section_html = $CI->Barcode_model->get_student_enrolled_section_edit($root->student_id, $root->section_id);

                        $tuitionFee = null;
                        if ($section) {
                            $tuition_data = ($section) ? $CI->Barcode_model->search_student_payment_status_for_update($root->student_id, $root->id, $root->section_id, $section->course_type) : null;
                            $settings = $this->db->select('tuition_fee_settings.tuition_fee, tuition_fee_settings.course_fee')
                            ...
                            if ($section->course_type==1) {
                                $tuitionFee = $settings->tuition_fee;
                            ...
                            } elseif($section->course_type==2) {
                                $tuitionFee = $settings->course_fee;
                                $query = $this->db->select('month, invoice_id')
                                    ->from('tuition_fee_collection')
                                    ->where('student_id', $root->student_id)
                                    ->where('section_id', $root->section_id)
				...
				<div class="row">
                        ...
                        <div class="panel panel-default panel-shadow" data-collapsed="0">
                            ...
                            <div class="panel-body">
                                ...
                                <?php
                                    foreach ($tui_pay as $t_inv)
                                    {
                                        $section = $this->db->select('course_type', 'name', 'nick_name')->from('section')
                                            ->where('section_id', $t_inv->section_id)->where('vtype', $vtype)->get()->row();
                                        $section_html = $CI->Barcode_model->get_student_enrolled_section_edit($root->student_id, $t_inv->section_id);
                                        $tuitionFee = null;
                                        if ($section) {
                                            $tuition_data = ($section) ? $CI->Barcode_model->search_student_payment_status_for_update($root->student_id, $root->id, $t_inv->section_id, $section->course_type) : null;
                                            $settings = $this->db->select('tuition_fee_settings.tuition_fee, tuition_fee_settings.course_fee')
                                                ->from('tuition_fee_settings')
                                                ->join('enroll', 'enroll.class_id=tuition_fee_settings.class_id')
                                                ->where('enroll.student_id', $root->student_id)
                                ...
                                ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col-sm-9">
                                            <input type="text" class="form-control" name="amount_paid_tuition[<?=$t_inv->section_id?>][]" id="paid_tuition" onkeyup="return isNumber(event, this.value, <?=$t_inv->section_id?>)"
                                                   value="<?php echo (empty($t_pay)) ? '' : $t_inv->amount_paid; ?>" placeholder="<?php echo get_phrase('enter_payment_amount');?>"/>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="col-sm-3 control-label"><?php echo get_phrase('discount');?></label>
                                        <div class="col-sm-9">
                                            <input type="text" class="form-control" value="<?php echo (empty($t_pay)) ? '0' : $t_pay->discount; ?>" name="discount_tuition[]" onkeyup="return isNumber(event, this.value, '<?=$t_inv->section_id?>')" placeholder="<?php echo get_phrase('enter_discount');?>" />
                                        </div>
                                    </div>
                                    <?php echo (empty($t_pay)) ? '' : '<input type="hidden" name="tui_id['.$t_inv->section_id.']" value="'.$t_pay->invoice_id.'">'; ?>
                                <?php
                                    }
                                ?>
...
<script>
    function isNumber(evt, value, key = '') {
        var ID = $(evt.target).attr("id");
        if ((ID === 'paid_tuition' + key) || ID === 'paid_exam' || ID === 'paid_session' || ID==='paid_transport' || ID==='paid_dress' || ID==='paid_others') {
            if (value>0) {
...
<script>
    function getTuitionFeeViewData(sec_id) {
        if (sec_id != '') {
            ...
            var current = '<?php echo $root->id; ?>';
            // var sec_id = jQuery('#section_id').val();
            $.ajax({
                url: '<?php echo site_url('admin/search_student_payment_status/');?>' + sec_id + '/' + std_id + '/' + current,
                success: function (response) {
                    jQuery('#monthly-tuition-data2').empty();
                    jQuery('#monthly-data').empty();
