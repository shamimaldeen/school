...
class Login extends CI_Controller {
    ...
    function validate_login() {
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('admin_id', $row->admin_id)->insert('admin', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('admin/dashboard'), 'refresh');
      }
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('teacher_id', $row->teacher_id)->insert('teacher', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('teacher/dashboard'), 'refresh');
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('student_id', $row->student_id)->insert('student', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('student/dashboard'), 'refresh');
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('parent_id', $row->parent_id)->insert('parent', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('parents/dashboard'), 'refresh');
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('librarian_id', $row->librarian_id)->insert('librarian', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('librarian/dashboard'), 'refresh');
      ...
      if ($query->num_rows() > 0) {
          ...
          if ($row->vtype == 0){
              $this->session->set_userdata('vtype', 1);     //For First Time;
              $this->db->where('accountant_id', $row->accountant_id)->insert('accountant', array('vtype' => 1));
          }else{
              $this->session->set_userdata('vtype', $row->vtype);
          }
          redirect(site_url('accountant/dashboard'), 'refresh');
